  ...
  render() {
    return (
      <div className="container">
        <h1 className="insertTitle">Insert a comment here</h1>
        {this.state.error &&  /*inline conditional rendering*/
          <Alert className="alert-box" bsStyle="warning">
            <strong>Need to login to comment on a professor <Link to="/login">Log In</Link></strong>
          </Alert>
        }
        ...
        <Form horizontal className="form">
          ...
          <button aria-label="submit button" type="submit" className="submit btn btn-primary" onClick={this.insertComment}>
            Submit
                    </button>
        </Form>
      </div>
